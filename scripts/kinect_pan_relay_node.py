...
class KinectPanRelayNode(Node):
    def __init__(self): # Corrected from _init
        super().__init__('kinect_pan_relay_node') # Corrected from _init

        self.declare_parameter('goal_angle_topic', '/kinect_pan_angle_goal',
                                ParameterDescriptor(description='Topic to subscribe for desired pan angle (Float64).'))
        self.declare_parameter('controller_command_topic', '/kinect_pan_controller/commands',
                                ParameterDescriptor(description='Topic to publish controller commands (Float64MultiArray).'))
        self.declare_parameter('joint_name', 'kinect_pan_joint',
                                ParameterDescriptor(description='Name of the pan joint.'))
    ...
    def goal_callback(self, msg: Float64):
        ...
        controller_command.data = [msg.data] # Target position for the kinect_pan_joint

        self.publisher_.publish(controller_command)
        self.get_logger().debug(f"Relayed pan command: {msg.data:.3f} rad for joint '{self.joint_name}'")

def main(args=None):
    rclpy.init(args=args)
    node = KinectPanRelayNode()
    try:
        rclpy.spin(node)
